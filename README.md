...
- **Campsite names/booking links** (`src/config/campsites.json`, admin id → `{ slug, name }`): resolved on demand straight from `GET v21/administrations/{numericAdminId}`, which — unlike the slug-keyed or accommodation-keyed `v21` endpoints — works with the public token. `toCampMatch()` calls this for any admin id not already in the file, so the table fills itself in over time; nothing needs to run separately. Only `status: "active"` administrations are accepted. A match whose campsite isn't resolved yet shows as `Admin {id}` with no booking link, rather than a guessed link that 404s.
